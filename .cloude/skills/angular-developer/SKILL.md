...
description: Generates Angular code and provides architectural guidance. Trigger when creating projects, components, or services, or for best practices on reactivity (signals, linkedSignal, resource), forms, dependency injection, HTTP (HttpClient, interceptors, httpResource), routing, SSR, accessibility (ARIA), animations, styling (component styles, Tailwind CSS), performance (@defer, zoneless), security, i18n, testing, or CLI tooling.
license: MIT
metadata:
  author: Copyright 2026 Google LLC
  version: '1.1'
  upstream: angular/skills (angular-developer), synced 2026-07-17
  localChanges: project-integration section (sistema-dsc-frontend), pnpm commands, added references (http, performance, security, i18n)
---
...
3. When you finish a task that generated or modified code, run the project's build script (`pnpm build` in this repository; `ng build` otherwise) to ensure there are no build errors. If there are errors, analyze the error messages and fix them before proceeding. Do not skip this step, as it is critical for ensuring the generated code is correct and functional.

## Project Integration (sistema-dsc-frontend)

This repository has a binding development contract in `agents.md` (repo root) and a short system prompt in `.cloude/system-prompt.md`. **If this skill and the contract conflict, the contract wins.** Key local rules:

- **Package manager**: pnpm. Use `pnpm start`, `pnpm build`, `pnpm test`, and `pnpm dlx @angular/cli` instead of npm/npx.
- **TypeScript-only**: no application `.js` files. Components use **inline** `template`/`styles` in the `.ts` file — never create `*.component.html` (`angular.json` schematics already enforce `inlineTemplate`/`inlineStyle`).
- **Architecture**: `core/` (singletons, no UI), `shared/` (dumb components, domain-agnostic), `features/` (lazy-loaded routes), `layout/` (shell). Path aliases: `@core/*`, `@shared/*`, `@features/*`, `@layout/*`.
- **State**: Signals for local/UI state; RxJS for async streams. No global stores without an ADR in `docs/adr/`.
- **Documentation**: JSDoc blocks are mandatory for every new service, component, public interface, guard, interceptor, and complex function.
- **Tailwind caveat**: Tailwind v4 ignores folders named `home` when scanning for classes. Name features `landing`, `auth`, `dashboard`, etc. — never `home`.
...
When communicating with backend services over HTTP, consult the following reference:

- **HttpClient, Interceptors, and httpResource**: Configuring `provideHttpClient`, functional interceptors, and reactive data fetching with `httpResource`. Read [http.md](references/http.md)
...
When optimizing rendering and load performance, consult the following reference:

- **Deferred Loading, Zoneless, and Images**: `@defer` blocks, zoneless change detection, `OnPush`, and `NgOptimizedImage`. Read [performance.md](references/performance.md)

## Security

When handling user-provided content, URLs, or raw HTML, consult the following reference:

- **Sanitization and Trusted Types**: Angular's built-in XSS protection, `bypassSecurityTrust*` risks, and CSP. Read [security.md](references/security.md)

## Internationalization (i18n)

When localizing an application, consult the following reference:

- **Localization with @angular/localize**: Marking text with `i18n`/`$localize`, extracting messages, and per-locale builds. Read [i18n.md](references/i18n.md)
